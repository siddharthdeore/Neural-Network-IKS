...
if __name__ == '__main__':
    ...
    dataset_train = np.loadtxt(absolute_file_path('datasets/test-large.csv'), delimiter=",")
    x_train=dataset_train[:,:2] # (input vector) first two columns 
    y_train=dataset_train[:,2:] # (output vector) second and third column
    # load test dataset
    dataset_test = np.loadtxt(absolute_file_path('datasets/test.csv'), delimiter=",")
    x_test=dataset_test[:,:2]
    y_test=dataset_test[:,2:]
    # prediction dataset
    dataset_predict = np.loadtxt(absolute_file_path('datasets/predict.csv'), delimiter=",")
    ...
    #model.save(absolute_file_path('models/model_4.h5'))  # creates a HDF5 model file 
